...
- Move acknowledgement (p95): under 100–200 ms.
- Opponent move delivery (p95): under 200–500 ms.
...
* Average active users per minute: 1,000,000 / 1440 ≈ 700–900 users per minute.
...
`20,000 pending requests × 1 KB = 20 MB`

This is small enough to keep in memory.

If the matching engine uses a Balanced BST, TreeSet, Redis Sorted Set, or similar range-query structure, lookup complexity is O(log N). For N = 20,000 pending requests, log2(20,000) ≈ 14 operations. So the matcher is not likely to be the hardest bottleneck.
...
`200,000 / 10 = 20,000 moves per second`
...
As wait time increases, we can gradually widen the rating range (e.g., first 5 seconds: ±100 rating, next 10 seconds: ±200 rating, etc.). This improves both fairness and match availability.
...
`Δt = T_received - T_last_committed`

The server subtracts Δt from the moving player's clock, applies any increment (if the time control uses increment), and commits the updated clock state.
...
`shard = hash(gameId) % number_of_shards`
...
`t_wait = min(t_max, 2^attempt * base) ± random_jitter`
...
- **Gameplay**: Move submit latency (p50, p95, p99), move validation latency, opponent delivery latency.
